  ...
  return (
    <div className="card hover:shadow-md transition-shadow">
      ...
      <div className="flex flex-wrap gap-1 mb-4">
        {curriculo.habilidades.slice(0, 4).map((h) => <span key={h} className="badge">{h}</span>)}
        {curriculo.habilidades.length > 4 && <span className="badge-gray">+{curriculo.habilidades.length - 4}</span>}
      </div>
      <div className="flex items-center justify-between pt-3 border-t border-gray-100">
        <span className="text-xs text-gray-400">{curriculo.experiencias.length} experiência(s)</span>
        <Link href={`/curriculos/visualizar/${curriculo.id}`} className="btn btn-primary text-xs py-1.5 px-3">
